...
//
  ...
  /////////////// eRx payload + header generation   (ECOND packet) ///////////////
  ...
  ///////////////ooooooooooooOOOOOOOOOOOOOOOOO CB Header OOOOOOOOOOOOOOoooooooooooooooooooo////////////////////
    ...
    if(ECOND_counter % 2 == 0){ // 2 ECOND per CB
      ...
      auto econdHeader = hgcal::econd::eventPacketHeader(header,
                                                              payloadLength[econdIdx], // since we are not have trailer in the econdpacket so the payload length is reduced by 1 (ideally it should be (37+2)*6 + 1 = 235)
                                                              false,
                                                              true,
                                                              ht,
                                                              ebo,
    ...
    if (!eRxPresent) {
      ...
      const auto erx_chan_data = hgcal::econd::produceERxData(enableMaps[erx], allERxData[erx], passZS, passZSm1, hasToA, charMode);  //eRx data for each eRX
      ///int i = 0;
...
cout << "payloadBytes: "  << payloadBytes << "  totalSize: " << totalSize << endl;
