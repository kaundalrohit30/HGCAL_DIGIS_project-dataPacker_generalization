...
class rawDataDumpAnlzr : public edm::one::EDAnalyzer<edm::one::SharedResources> {
public:
  explicit rawDataDumpAnlzr(const edm::ParameterSet&);
  ~rawDataDumpAnlzr() override;
...
//
rawDataDumpAnlzr::rawDataDumpAnlzr(const edm::ParameterSet& iConfig)
...
rawDataDumpAnlzr::~rawDataDumpAnlzr() {
...
void rawDataDumpAnlzr::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  using namespace edm;
  using namespace std;
  ...
  uint32_t econdHeader, econdHeaderPos, econdHeaderMask, econdPayloadPos, econdPayloadMask, econdHeaderMarker, econdPayloadLength, econdTrailer;
  std::vector<uint32_t> originalCRC, repackedCRC;
  ...
  for (const auto& it : rawdata2->map()) {
    ...
    if (dumpPayload_) {
      const uint32_t* payload = (uint32_t*)(start_fed_data);
      cout << std::hex << std::setfill('0');
      size_t step1 = 1;
      for (unsigned int i = 0; i < size / sizeof(uint32_t); i+=step1) {
        ...
        if(econdHeaderMarker == econdHeader){
          ...
          if(econdPayloadLength % 2 == 0)
            econdTrailer = payload[i+econdPayloadLength-1];
          else  
            econdTrailer = payload[i+econdPayloadLength+1];
...
void rawDataDumpAnlzr::beginJob() {
  // please remove this method if not needed
}

// ------------ method called once each job just after ending the event loop  ------------
void rawDataDumpAnlzr::endJob() {
  // please remove this method if not needed
}

// ------------ method fills 'descriptions' with the allowed parameters for the module  ------------
void rawDataDumpAnlzr::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
DEFINE_FWK_MODULE(rawDataDumpAnlzr);
