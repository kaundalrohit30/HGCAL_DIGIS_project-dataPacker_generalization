...
private:
  ...
  //TH1D* h_adc, h_adcm1, h_tot, h_toa, h_tctp, h_cm;
  //TH1D* h_adc;
  //TH1D* h_adcm1;
  //TH1D* h_tot;
  //TH1D* h_toa;
...
testRAW2DIGI_validation::testRAW2DIGI_validation(const edm::ParameterSet& iConfig)
  ...
  //h_adc = fs->make<TH1D>("ADC", "ADC distribution;ADC;Counts", 1024, 0, 1024);
  //h_adcm1 = fs->make<TH1D>("ADCm1", "ADCm1 distribution;ADCm1:Counts", 1024, 0, 1024);
  //h_tot = fs->make<TH1D>("TOT", "TOT distribution;TOT;Counts", 1024, 0, 1024);
  //h_toa = fs->make<TH1D>("TOA", "TOA distribution;TOA;Counts", 5000, 0, 5000);
  //h_tctp = fs->make<TH1D>("TcTp", "TcTp distribution;TcTp;Counts", 5, 0, 5);
  //h_cm = fs->make<TH1D>("CM", "CM distribution;CM;Counts", 1024, 0, 1024);
}
